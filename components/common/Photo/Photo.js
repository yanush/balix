...
export default class Photo extends Component {
  ...
  getDetailsFromProps() {
    ...
    let userData = userService.getUserById(this.props.data.userId);
    let userImages = imageService.getUserImages(userData.userId);
    this.setState((prevState) => {
      return {
        ...prevState,
        imageData: imageData,
  ...
  render() {
    if(this.state.userData && this.state.imageData) {
      return (
        <ScrollView style={styles.container}>
          ...
          <View style={styles.photoBox}>
            ...
            <PhotoIndicator indicators={{cash: this.state.imageData.cash + this.state.plusCash, hearts: this.state.imageData.hearts}}/>
            <View style={styles.emoji}>
              {
                (this.state.openEmoji) ? (<EmojiBox emojiSize={this.emojiSize} emojiPress={this.emojiPress.bind(this)} />) : (<View></View>)
              }
            </View>
              <Animated.Image
          ...
          <View style={styles.buttonsBox}>
              <View style={styles.leftSide}>
                <TouchableHighlight onPress={this.toggleEmoji.bind(this)}>
                  <Icon style={styles.icon} name={iconNames.LIKE} size={Style.sizes.icon} color={Style.colors.icon} />
                </TouchableHighlight>
                <Icon style={styles.icon} name={iconNames.COMMENT} size={Style.sizes.icon} color={Style.colors.icon} />
              </View>
              <View style={styles.rightSide}>
                <Icon style={styles.icon} name={iconNames.SHARE} size={Style.sizes.icon} color={Style.colors.icon} />
              </View>
            </View>
          <View style={styles.anotherPhotos}>
            {
              this.state.userImages.map((img, i) => {
                ...
                return (
                    <TouchableHighlight key={i} style={{padding: 5, width: '12.5%', aspectRatio: 1/1}} onPress={() => this.setState({imageData: img})} >
          ...
          <View style={styles.commentsBox}>
            ...
            <TouchableHighlight 
              ...
              <Text style={styles.allCommentsLink}>View {withComma(this.state.comments.length)} Comments</Text>
            </TouchableHighlight>
